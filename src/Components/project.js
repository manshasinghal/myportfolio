...
export default function Project() {
  ...
  return (
    ...
    <LazyMotion features={domAnimation} strict> 
      ...
      >
      <div className='grid grid-cols-1 lg:grid-cols-3  align-center ml-[70px]'>
    
        {List2.map(({ bg, name ,para , link1 , link2, tech}, index) => (
          <div
            key={index}
            className=' mt-10  sm:gap-10 relative ml-[-40px] group w-[350px] h-[500px]  mt-8  hover:opacity-100 transition-opacity duration-300 ease-in-out bg-slate-950 border-slate-900 shadow-md shadow-slate-400'
           
            onMouseEnter={() => setHoveredIndex(index)}
            onMouseLeave={() => setHoveredIndex(null)}
          >
           <img className='w-[350px] h-[250px] ' src={bg} alt="" />
           <p className='text-white text-center mt-4 font-semibold lg:font-bold text-xl'>{name}</p>
           <p className='text-white  mt-4 font-normal text-[16px] ml-2 '>{para}</p>
          ...
          <div
            ...
            >
                ...
                <div className='bg-gradient-to-r from-white via-orange-500 to-white w-[350px] h-[250px]  pt-28 opacity-100 transition-opacity ease-in duration-300 '>
